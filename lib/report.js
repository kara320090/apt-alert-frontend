...
function safeDiscountRate(item) {
  return Math.max(0, Number(item?.discount_rate || 0));
}
...
export function buildRegionReport(listings, selectedRegion = "전체") {
  ...
  const avgDiscount =
    totalCount > 0
      ? (
          regionFiltered.reduce((sum, item) => sum + safeDiscountRate(item), 0) /
          totalCount
        ).toFixed(1)
      : "0.0";
  ...
  for (const item of regionFiltered) {
    ...
    row.count += 1;
    row.discountSum += safeDiscountRate(item);

...
// 이메일 알림 구독
export async function subscribeAlert({ email, region, grade = "전체", minDiscount, signal }) {
...
// 현재 페이지 매물 AI 태그/요약
export async function fetchAiListingTags(listings, { signal } = {}) {
...
// 지역 요약 Gemini 생성
export async function fetchRegionAiSummary({ report, selectedRegion, signal } = {}) {
  const res = await fetch("/api/ai/region-summary", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    signal,
    body: JSON.stringify({ report, selectedRegion }),
  });
